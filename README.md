...
| **레이저 사격** | `Space` | 전방으로 레이저를 발사하며 8비트 비프 사운드가 재생됩니다. |
| **이니셜 입력** | `A` ~ `Z` | 게임 종료 시 알파벳 3글자를 입력합니다. |
| **글자 수정** | `Backspace` | 입력 중인 이니셜을 한 글자 지웁니다. |
| **랭킹 등록** | `Enter` | 3글자 이니셜 등록을 확정하고 랭킹 보드를 확인합니다. |
| **재시작** | `R` | 게임오버 또는 미션 승리 랭킹 화면에서 다시 시작합니다. |
| **게임 종료** | `ESC` | 게임 창을 즉시 닫습니다. |

### 🏆 핵심 게임 규칙
* **생명 시스템 (3개)**: 시작 시 3개의 생명이 주어지며, 화면 **오른쪽 상단에 숫자(`LIVES: 3`)**로 표시됩니다. 피격 시 1개씩 감소하며 0개가 되면 게임 오버가 됩니다.
* **명예의 전당 (영구 랭킹)**: 게임 종료 후 3글자 이니셜을 입력하면 점수와 함께 `ranking.json`에 영구 저장되어 게임을 껐다 켜도 순위표가 유지됩니다.
* **8비트 레이저 발사음**: 스페이스바로 탄환을 발사할 때 청명한 아케이드 피치 사운드가 즉각 출력됩니다.
...
```text
star-invader/
├── assets/                  # 픽셀 아트 이미지 및 효과음
│   ├── images/              # 배경, 플레이어, 적 함선, 폭발 스프라이트
│   ├── sounds/              # 8비트 아케이드 효과음 (laser_shoot.wav)
│   └── ASSET_GUIDE.md       # 그래픽 리소스 프롬프트 및 규격 문서
├── constants.py             # ⚙️ 게임 설정 및 수치값 (화면 크기, 속도, 색상 등)
├── resource_loader.py       # 📦 스마트 배경 투명화 및 스프라이트 로더
├── sound_manager.py         # 🔊 8비트 비프음 자동 합성 및 효과음 관리자
├── ranking_manager.py       # 🏆 3글자 이니셜 랭킹 영구 저장(JSON) 관리자
...
├── ranking.json             # 💾 영구 보존되는 명예의 전당 랭킹 데이터
